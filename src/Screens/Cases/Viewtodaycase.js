...
const Viewtodaycases = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true); // Ensure loading state is true when the request starts
                const response = await axios.get(`${urlapi}/api/v1/auth/gettodayentries`);
                // If response.data is an array and it's not empty, set entries
                if (Array.isArray(response.data) && response.data.length > 0) {
                    setEntries(response.data);
                } else {
                    // If there are no entries, you can set an empty array here
                    setEntries([]);
                }
            } catch (error) {
                console.error('Error fetching data: ', error);
                setEntries([]); // In case of error, set entries to an empty array
            } finally {
